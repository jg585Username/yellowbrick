...
class TestFeatureVisualizer(object):
    ...
    def test_fit_transform_show(self):
        ...
        viz.fit.assert_called_once_with(X, y)
        # fit_transform calls transform(X) via sklearn's TransformerMixin, not transform(X, y)
        viz.transform.assert_called_once_with(X)
